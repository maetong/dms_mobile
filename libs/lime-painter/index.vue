...
export default {
	...
	data() {
		...
		return {
			canvasId,
			use2dCanvas: this.type ==='2d' ? true : false ,// 微信 2.9.2 后可用canvas 2d 接口,
			draw: null,
			ctx: null
		};
	},
	...
	methods: {
		render(args = {}) {
			this.getContext().then(async (ctx) => {
              ...
              console.log('canvasToTempFilePath base64ToPath ', img);
							this.$emit('success', img)
						} else {
              console.log('canvasToTempFilePath tempFilePath', res);
